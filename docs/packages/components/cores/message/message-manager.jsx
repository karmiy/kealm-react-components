...
import { isObject, isEmpty, isNumber, isFunction } from 'utils/common/base';
...
const MessageManager = {
    ...
    getWrap() {
        ...
        const container = base_container();
        ...
        const messageComps = this.items.map(item => {
            return (
                <MessageTrigger
                    ...
                    {...item.props}
                    onMount={el => {
                        this.onMount(el, item.key);
                        const { onMount } = item.props;
                        onMount && onMount(el);
                    }}
...
const api = {
    ...
    config(config) {
        const { top, duration, maxCount, getContainer } = config;
        !isEmpty(top) && isNumber(top) && (base_top = top);
        !isEmpty(duration) && isNumber(duration) && (base_duration = duration);
        !isEmpty(maxCount) && isNumber(maxCount) && (base_max_count = maxCount);
        !isEmpty(getContainer) && isFunction(getContainer) && (base_container = getContainer);
    },
    destroy: MessageManager.closeAll.bind(MessageManager),
}
